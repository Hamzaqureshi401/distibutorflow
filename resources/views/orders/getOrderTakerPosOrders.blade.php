                ...
                <div class="row">
                  ...
                  <div class="col-md-12">
                    ...
                    @if(Auth::user()->role <3) 
                      ...
                      <button type="button" class="btn btn-primary btn-sm confirm-btn send-to-unapprove" id="dis">Confirm</button>
                       <button type="button" @if(Auth::user()->role < 3) data-toggle="modal" data-target="#order-unapprove-popup" @else onclick="document.getElementById('multiple-approve').submit()" @endif class="btn btn-success btn-sm app-mult">Change seller</button>
        ...
        <div class="table-responsive">
          <table class="table table-bordered table-custom-th table-datatable table-hover" id="dataTable" width="100%" cellspacing="0">
            ...
            <tbody>
              ...
              @if ($order->use_stock == 0 )          <tr class="dont_use_stock set-background">    
             
             @elseif ( $order->pick_order == 1 )     <tr class="pickorder set-background">
            
            
             @elseif ( $order->cancel_status == 0 )     <tr class="cancel set-background">
              
             @elseif ( $order->cancel_status == 2 ) <tr class="delivery-failed set-background">
                   
             @elseif($order->clear == 'clear')        <tr class="clear set-background">
                        
             @elseif( $days_between == 0 )          <tr class="today-delivery set-background">
                        
             @elseif($days_between == -1)           <tr class="delivery-missed set-background">
                        
             @elseif($days_between < 1)             <tr class="delivery-missed-2day set-background">
                         
             @elseif( $order['bill_no'] == 0 )      <tr class="new-customer set-background">
                        
             @elseif($days_between > 0)             <tr class="set-background">
...
<td class="text-center">
                           ...
                           <span>Map view</span><br><input type="checkbox" data-toggle="toggle" data-onstyle="success" data-size="xs" value="{{ $order->customers->location_url }}" data-rowid="{{ $order->id }}" class="add-coords js-switch" />
                           </label>
                           @if(Auth::user()->role < 3)
               ...
               @if ( $order->received_amount < $order->subtotal  )
                        <td class="text-center" style="color: red" data-changein="subtotal">
                         ...
                         <br>{{ $order->customer->user->name }}<br>
                           @if( $order->clear == NULL )
       ...
       <div class="card-footer small text-muted filtered-by-seller">
          ...
          |
          Discount: <b>{{ $orders->sum('discount') }}</b> 
          | 
              ...
              @foreach($product_report as $preport)
                ...
                @if($preport['amount'] != 0 || $preport['unit'] != 0)
                  <tr>
                    ...
                    <td>{{ $preport['unit'] }}</td>
                    <td>{{ $preport['stock'] }}</td>
                    <td>{{ $preport['unit'] }} - {{ $preport['stock'] }} = {{ $preport['stock'] - $preport['unit'] }}</td>
                    @if(Auth::user()->role < 3)
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" action="" class="category-form">
          ...
          <h5>Send Urgent</h5>
          <input class="js-switch" type="checkbox" name="urgent"  >
          <br>
...
<div class="modal fade" id="order-unapprove-popup" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button class="btn btn-secondary confirm-btn">Seller Confirm</button>
         @if(Auth::user()->role < 3)
        <button class="btn btn-primary confirm-btn send-to-unapprove">Pending Invoices</button>
        @endif
      </div>
    </div>
  </div>
