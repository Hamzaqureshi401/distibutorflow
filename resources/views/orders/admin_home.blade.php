            ...
            <!--Unit-->
                ...
                <td>
                  ...
                  <a href="javascript:;" data-toggle="modal" data-target="#invoice-detail-popup" class="btn btn-sm btn-success view-details" id="{{ $invoice->id }}"><i class="fa fa-eye"></i></a>
                    @if (Auth::user()->role < 3 && $invoice->received_amount == 0)
                      <a href="{{ route('equal.invoice' , $invoice->id) }}" class="btn btn-sm btn-warning"><i class="fa fa-edit"></i></a>
                      @endif
                   @if($conditiont[$counter]->id == $invoice->id)
                  @if(Auth::user()->role < 3)
                  
                 <a href="{{ route('print.invoice' , $invoice->id) }}" class="btn btn-sm btn-info"><i class="fa fa-print"></i></a>
                  <a href="{{ route('customer.invoices' , $invoice->customer_id) }}" class="btn btn-sm prev-record btn-primary"><i class="fa fa-reply"></i></a>
